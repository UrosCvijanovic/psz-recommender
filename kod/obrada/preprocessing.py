...
def u_slug(s):
    ...
    s = s.lower().strip()
    # sve sto nije slovo ili broj pretvara u -, ako na pocetku i na kraju ima neki simbol, stripuje ga
...
def preprocess_engine():
    ponude = db.ucitaj_ponude(sema=RAW_SHEMA, tabela="ponuda_raw")
    ...
    print(f"van opsega kategorija: {pre_kat - posle_kat} | manje od 3 specifikacije: {posle_kat - len(ponude)}")
    return ponude, specs
